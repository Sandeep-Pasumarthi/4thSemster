...
int main(int argc, char *argv[])
{
    ...
    int *img, *close_img, *structure;
    img = (int *)malloc(row * col * sizeof(int));
    close_img = (int *)malloc(row * col * sizeof(int));
    ...
    for (i = 0; i < row; i++)
    {
        for (j = 0; j < col; j++)
        {
            fscanf(fp, "%d", &temp);
            *(img + i * col + j) = temp;
            *(close_img + i * col + j) = temp;
        }
    }

    closing(close_img, structure, row, col, x, y);

    for (i = 0; i < row; i++)
    {
        for (j = 0; j < col; j++)
        {
            fprintf(op, "%d ", *(close_img + i * col + j));
        }
        fprintf(op, "\n");
    }
